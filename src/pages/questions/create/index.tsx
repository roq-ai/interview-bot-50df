...
import { getJobDomains } from 'apiSdk/job-domains';
...
function QuestionCreatePage() {
  ...
  const formik = useFormik<QuestionInterface>({
    initialValues: {
      content: '',
      job_domain_id: (router.query.job_domain_id as string) ?? null,
    },
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<JobDomainInterface>
            formik={formik}
            name={'job_domain_id'}
            label={'Select Job Domain'}
            placeholder={'Select Job Domain'}
            fetcher={getJobDomains}
            renderOption={(record) => (
