...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createQuestion() {
    ...
    const body = { ...req.body };
    if (body?.candidate_response?.length > 0) {
      const create_candidate_response = body.candidate_response;
      body.candidate_response = {
        create: create_candidate_response,
      };
    } else {
      delete body.candidate_response;
    }
